...
#
# io_v2 port of LightRedmule. Identical GEMM datapath / FSM; the CSR-input and
# TCDM ports speak the forked io_v2 protocol (IO_REQ_DONE / GRANTED / DENIED,
# resp()/retry()) so it can drop into the io_v2 teranoc_v2 tile behind the
# io_v2 TeranocHWPEInterleaver. See light_redmule_v2.cpp for the v1->v2 seam.

import gvsoc.systree

class LightRedmuleV2(gvsoc.systree.Component):

    def __init__(self, parent: gvsoc.systree.Component, name: str, tcdm_bank_width: int,
            tcdm_bank_number: int, elem_size: int, ce_height: int, ce_width: int, ce_pipe: int,
            queue_depth: int=128, fold_tiles_mapping: int=0,
        ...
        self.add_properties({'tcdm_bank_width'   : tcdm_bank_width,
            'tcdm_bank_number'  : tcdm_bank_number, 'elem_size'         : elem_size,
            # 'ce_height'         : ce_height,
            # 'ce_width'          : ce_width,
            ...
            'ce_height'         : ce_width, 'ce_width'          : ce_height,
            'ce_pipe'           : ce_pipe, 'queue_depth'       : queue_depth,
            'fold_tiles_mapping': fold_tiles_mapping, 'loc_base'          : loc_base,})

    def i_INPUT(self) -> gvsoc.systree.SlaveItf:
        return gvsoc.systree.SlaveItf(self, 'input', signature='io_v2')

    def i_INPUT_V2(self) -> gvsoc.systree.SlaveItf:
        return gvsoc.systree.SlaveItf(self, 'input_v2', signature='io_v2')
    ...
    def o_TCDM(self, itf: gvsoc.systree.SlaveItf):
        self.itf_bind('tcdm', itf, signature='io_v2')
