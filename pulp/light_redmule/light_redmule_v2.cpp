...
class LightRedmuleV2 : public vp::Component
{

public:
    LightRedmuleV2(vp::ComponentConf &config);
...
// private: ? why not private?????
    ...
    // TCDM io_v2 response / retry callbacks.
    ...
    vp::Trace           trace;
    vp::IoSlave         input_itf{&LightRedmuleV2::req};
    vp::IoSlave         input_itf_v2{&LightRedmuleV2::req_v2};
    ...
    vp::IoMaster        tcdm_itf{&LightRedmuleV2::tcdm_retry, &LightRedmuleV2::tcdm_response};
    ...
    struct ReqSlot {
        ...
        uint32_t        dst_offset;
        uint32_t        cutoff;
        // <=now: free. >now (finite): sync in-flight until that cycle.
        // INT64_MAX: async GRANTED/DENIED awaiting callback.
        int64_t         available_at;
        // Issue-time address, restored on a synchronous io_v2 retry re-send.
        uint64_t        orig_addr;
    };
    std::vector<ReqSlot> req_slots;
    // io_v2 resp/retry correlation: request object -> slot index.
    std::unordered_map<vp::IoReq *, int> req_slot_index;
    bool                request_denied;  // last issue was DENIED, awaiting retry()
    int                 denied_slot_id;  // slot held for a synchronous retry re-send
...
{
    return new LightRedmuleV2(config);
}

LightRedmuleV2::LightRedmuleV2(vp::ComponentConf &config)
    : vp::Component(config),
    ...
    reg_busy(*this, "busy", 1, true, 0)
{
    ...
    this->offload_itf.set_sync_meth(&LightRedmuleV2::offload_sync);
    this->new_slave_port("offload", &this->offload_itf, this);

    // Declare offload master interface for granting blocked transfers
    //this->offload_grant_itf.set_sync_meth(&LightRedmuleV2::offload_grant);
    ...
    this->fsm_event         = this->event_new(&LightRedmuleV2::fsm_handler);
    ...
    for (int i = 0; i < pool_size; i++)
    {
        ...
        slot.available_at = 0;
        slot.orig_addr    = 0;
        this->req_slot_index[slot.req] = i;
    }

    this->trace.msg("[LightRedmuleV2] Model Initialization Done! (slot pool size=%d)\n", pool_size);
}

void LightRedmuleV2::init_redmule_meta_data(){
...
uint32_t LightRedmuleV2::next_iteration(){
    this->iter_k += 1;
...
uint32_t LightRedmuleV2::calculate_tile_base_address(uint32_t base, uint32_t stride, uint32_t tile_col, uint32_t tile_row, uint32_t i, uint32_t j){
    /*
    *       ----
    *   col |  |
    *       ----
...
uint32_t LightRedmuleV2::inc_addr(uint32_t addr, uint32_t stride, uint32_t tile_row){
    if (this->fold_tiles_mapping)
    {
...
uint32_t LightRedmuleV2::tmp_next_addr(){
    this->z_addr = (this->z_addr + this->bandwidth) % (this->ce_height * this->bandwidth);
    return this->z_addr;
}

uint32_t LightRedmuleV2::next_addr(){
    uint32_t addr       = 0;
    uint32_t buffer_h   = this->ce_height;
    uint32_t buffer_w   = this->ce_width * (this->ce_pipe + 1);
    uint32_t buffer_n   = this->bandwidth / this->elem_size;
    ...
    if (this->y_acc_block > 0)
    {
        ...
        this->y_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2][Address] Y tile at 0x%11x | #Y tile left %d\n", addr, this->y_acc_block);
    ...
    } else if (this->w_acc_block > 0)
    {
        ...
        this->w_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2][Address] W tile at 0x%11x | #W tile left %d\n", addr, this->w_acc_block);
        this->tcdm_req->set_is_write(0);
        this->tcdm_req->set_size(buffer_w * this->elem_size);
    ...
    } else if (this->x_acc_block > 0)
    {
        ...
        this->x_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2][Address] X tile at 0x%11x | #X tile left %d\n", addr, this->x_acc_block);
        this->tcdm_req->set_is_write(0);
    ...
    } else if (this->z_acc_block > 0)
    {
        ...
        this->z_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2][Address] Z tile at 0x%11x | #Z tile left %d\n", addr, this->z_acc_block);
        this->tcdm_req->set_is_write(1);
        this->tcdm_req->set_size(this->z_store_width * this->elem_size);
        this->iter_instruction  = INSTR_STOR_Z;
    } else {
        this->trace.fatal("[LightRedmuleV2][Address] INVALID redmule address iteration : No tiles to access\n");
    }

    return addr;
}

void LightRedmuleV2::process_compute(){
    uint32_t buffer_h           = this->ce_height;
    uint32_t buffer_w           = this->ce_width * (this->ce_pipe + 1);
    uint32_t buffer_n           = this->bandwidth / this->elem_size;
    ...
    {
        //UINT16
        matmul_uint16(  (uint16_t *)this->z_buffer_compute, (uint16_t *)this->z_buffer_compute,
            (uint16_t *)this->x_buffer, (uint16_t *)this->w_buffer, (uint16_t)buffer_h,
            (uint16_t)buffer_n, (uint16_t)buffer_w);
    } else
    if (this->compute_able == 2)
    {
        //INT16
        matmul_int16(   (int16_t *)this->z_buffer_compute, (int16_t *)this->z_buffer_compute,
            (int16_t *)this->x_buffer, (int16_t *)this->w_buffer, (uint16_t)buffer_h,
            (uint16_t)buffer_n, (uint16_t)buffer_w);
    } else
    if (this->compute_able == 3)
    {
        //FP16
        this->matmul_fp16(    (fp16 *)this->z_buffer_compute, (fp16 *)this->z_buffer_compute,
            (fp16 *)this->x_buffer, (fp16 *)this->w_buffer, (uint16_t)buffer_h, (uint16_t)buffer_n,
            (uint16_t)buffer_w);
    }
    if (this->compute_able == 5)
    {
        //UINT8
        matmul_uint8(   (uint8_t *)this->z_buffer_compute, (uint8_t *)this->z_buffer_compute,
            (uint8_t *)this->x_buffer, (uint8_t *)this->w_buffer, (uint16_t)buffer_h,
            (uint16_t)buffer_n, (uint16_t)buffer_w);
    }
    if (this->compute_able == 6)
    {
        //UINT8
        matmul_int8(    (int8_t *)this->z_buffer_compute, (int8_t *)this->z_buffer_compute,
            (int8_t *)this->x_buffer, (int8_t *)this->w_buffer, (uint16_t)buffer_h,
            (uint16_t)buffer_n, (uint16_t)buffer_w);
    }
    if (this->compute_able == 7)
    {
        //UINT8
        matmul_fp8e4m3(     (fp8e4m3 *)this->z_buffer_compute, (fp8e4m3 *)this->z_buffer_compute,
            (fp8e4m3 *)this->x_buffer, (fp8e4m3 *)this->w_buffer, (uint16_t)buffer_h,
            (uint16_t)buffer_n, (uint16_t)buffer_w);
    }

}

void LightRedmuleV2::process_iter_instruction(){
...
void LightRedmuleV2::process_iter_instruction(uint8_t *buf, uint32_t instr){
...
uint32_t LightRedmuleV2::get_routine_access_block_number(){
...
uint32_t LightRedmuleV2::get_preload_access_block_number(){
    uint32_t total_blocks       = 0;
...
uint32_t LightRedmuleV2::get_storing_access_block_number(){
    uint32_t total_blocks       = 0;
    uint32_t buffer_h           = this->ce_height;
    uint32_t buffer_w           = this->ce_width * (this->ce_pipe + 1);
    uint32_t buffer_n           = this->bandwidth / this->elem_size;
...
uint32_t LightRedmuleV2::get_routine_to_storing_latency(){
    return this->ce_width * (this->ce_pipe + 1);
    // return 0;
}

uint32_t LightRedmuleV2::get_redmule_array_runtime(){
    uint32_t tcdms_bw       = this->bandwidth / this->elem_size;
...
uint32_t LightRedmuleV2::op_foramt_parser(uint32_t op_format) {
    ...
    if (operation > 1)
    {
        this->trace.fatal("[LightRedmuleV2] Unsupported operation %d (only MATMUL=0/GEMM=1) "
            "[op_format=0x%x]\n", operation, op_format);
    }
    if (data_format==1)
        compute_able=3;
        ...
        this->trace.fatal("[LightRedmuleV2] Unsupported data_format %d [op_format=0x%x]\n",
            data_format, op_format);
    return compute_able;
}

void LightRedmuleV2::offload_sync(vp::Block *__this, IssOffloadInsn<uint32_t> *insn)
{
    LightRedmuleV2 *_this = (LightRedmuleV2 *)__this;
    ...
    switch (opc)
    {
        case 0b0001011:
        {
            if (_this->reg_fsm_state.get() == IDLE) {
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] Set MNK size: %d, %d, %d\n", _this->m_size, _this->n_size, _this->k_size);
            }
            break;
        }
        case 0b0101011:
        {
            if (_this->reg_fsm_state.get() == IDLE)
            {
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] Set XWY addr: 0x%08x, 0x%08x, 0x%08x\n", _this->x_addr, _this->w_addr, _this->y_addr);

                /*************************
                *  Asynchronize Trigger  *
                *************************/
                //Sanity Check
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] redmule configuration (M-N-K): %d, %d, %d\n", _this->m_size, _this->n_size, _this->k_size);
                if ((_this->m_size == 0)||(_this->n_size == 0)||(_this->k_size == 0))
                {
                    _this->trace.fatal("[LightRedmuleV2] INVALID redmule configuration (M-N-K): %d, %d, %d\n", _this->m_size, _this->n_size, _this->k_size);
                }
...
int32_t LightRedmuleV2::acquire()
{
    if (this->job_state != 0) {
...
void LightRedmuleV2::commit(bool start)
{
    if (this->job_state == 0) {
    ...
    if (this->job_state != -2) {
        //Queue full: drop
        this->trace.msg(vp::Trace::LEVEL_WARNING,"[LightRedmuleV2] COMMIT_TRIGGER dropped: job queue full (job_pending=%d)\n", this->job_pending);
        return;
    }
...
void LightRedmuleV2::start_next_job()
{
    //Load context
    ...
    //Sanity Check
    this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] redmule configuration (M-N-K): %d, %d, %d. Compute able is %d\n", this->m_size, this->n_size, this->k_size, this->compute_able);
    if ((this->m_size == 0)||(this->n_size == 0)||(this->k_size == 0))
    {
        this->trace.fatal("[LightRedmuleV2] INVALID redmule configuration (M-N-K): %d, %d, %d\n", this->m_size, this->n_size, this->k_size);
        return;
    }
...
void LightRedmuleV2::soft_clear(uint32_t value)
{
    if ((value == 0) || (value == 2)) {
...
//LightRedmuleV2 register interface
vp::IoReqStatus LightRedmuleV2::req(vp::Block *__this, vp::IoReq *req)
{
    LightRedmuleV2 *_this = (LightRedmuleV2 *)__this;
    ...
    //_this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] access (offset: 0x%x, size: 0x%x, is_write: %d, data:%x)\n", offset, size, is_write, *(uint32_t *)data);
        ...
        switch (offset) {
            case 0x00: {
                if ((_this->redmule_query == NULL) && (_this->reg_fsm_state.get() == IDLE)) {
                    ...
                    ************************/
                    //Sanity Check
                    _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] redmule configuration (M-N-K): %d, %d, %d. Compute able is %d\n", _this->m_size, _this->n_size, _this->k_size,_this->compute_able);
                    if ((_this->m_size == 0)||(_this->n_size == 0)||(_this->k_size == 0))
                    {
                        _this->trace.fatal("[LightRedmuleV2] INVALID redmule configuration (M-N-K): %d, %d, %d\n", _this->m_size, _this->n_size, _this->k_size);
            ...
            case 0x40: {
                _this->x_addr = value;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] Set X addr 0x%x)\n", _this->x_addr);
                break;
            }
            case 0x44: {
                _this->w_addr = value;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] Set W addr 0x%x)\n", _this->w_addr);
                break;
            }
            case 0x48: {
                _this->y_addr = value;
                _this->z_addr = _this->y_addr;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] Set Y addr 0x%x, Set Z addr 0x%x)\n", _this->y_addr,_this->z_addr);
                break;
            }
            case 0x4C: { //REDMULE_MCFG0_PTR
                _this->m_size=value & 0xFFFF;
                _this->k_size=value >> 16;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] Set M size %d, Set K size %d)\n", _this->m_size,_this->k_size);
                break;
            }
            case 0x50: { //REDMULE_MCFG1_PTR
                _this->n_size=value & 0xFFFF;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] Set N size %d)\n", _this->n_size);
                break;
            }
            case 0x54: {
                ...
                _this->elem_size = (_this->compute_able < 4)? 2:1;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] marith 0x%x, compute_able 0x%x, elem_size %d)\n", value,_this->compute_able,_this->elem_size);
                break;
            }
            default:
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] write to INVALID address\n");
        }
    }
    else {
        switch (offset) {
            case 0x00:
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] read to INVALID address\n");
                break;
            case 0x0C: {
                int32_t done_id_r;
                if ((_this->redmule_query == NULL) && (_this->reg_fsm_state.get() == IDLE)) {
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] read status reg 0x%x\n",done_id_r);
                break;
            }
            default:
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] read to INVALID address\n");
...
vp::IoReqStatus LightRedmuleV2::req_v2(vp::Block *__this, vp::IoReq *req)
{
    LightRedmuleV2 *_this = (LightRedmuleV2 *)__this;
    ...
    if (is_write == 1) {
        ...
        switch (offset) {
            ...
            case 0x20: { //REDMULE_MCFG0_PTR
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] Set mcfg_reg0 -- M size %d, Set K size %d)\n", _this->cxt_m_size[_this->cxt_cfg_ptr],_this->cxt_k_size[_this->cxt_cfg_ptr]);
                break;
            }
            case 0x24: { //REDMULE_MCFG1_PTR
                _this->cxt_n_size[_this->cxt_cfg_ptr] = value & 0xFFFF;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] Set mcfg_reg1 -- N size %d)\n", _this->cxt_n_size[_this->cxt_cfg_ptr]);
                break;
            }
            case 0x28: { //REDMULE_MCFG2_PTR
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] Set mcfg_reg2 -- currently not used\n");
                break;
            }
            case 0x2C: {
            ...
            case 0x34: {
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] Set Y addr 0x%x)\n", _this->cxt_y_addr[_this->cxt_cfg_ptr]);
                break;
            }
            case 0x14: { //REDMULE_SOFT_CLEAR
                _this->soft_clear(value);
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] SOFT_CLEAR 0x%x\n", value);
                break;
            }
            case 0x54: {
                uint32_t gemm_op  = (value >> 10) & 0x7;
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] marith 0x%x, compute_able 0x%x)\n", value,_this->cxt_compute_able[_this->cxt_cfg_ptr]);
                break;
            }
            default:
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] write to INVALID address\n");
        ...
        switch (offset) {
            case 0x04: { //REDMULE_ACQUIRE
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] ACQUIRE -> job id %d\n", id);
                break;
            }
            ...
            case 0x10: { //REDMULE_RUNNING_JOB
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] RUNNING_JOB -> %d\n", _this->running_job_id);
                break;
            }
            default:
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2] read to INVALID address\n");
...
int LightRedmuleV2::alloc_req_slot()
{
    int64_t now = this->clock.get_cycles();
    int     best_id = -1;
...
void LightRedmuleV2::free_req_slot(int id, int64_t available_at)
{
    this->req_slots[id].available_at = available_at;
}

bool LightRedmuleV2::all_slots_free()
{
    int64_t now = this->clock.get_cycles();
...
bool LightRedmuleV2::issue_slot_gate_ok()
{
    int64_t now = this->clock.get_cycles();
...
// state is touched.
void LightRedmuleV2::apply_response_data(uint8_t *buf, uint32_t instr, uint32_t dst_offset, uint32_t cutoff)
{
    switch (instr)
    {
        case INSTR_LOAD_Y:
...
void LightRedmuleV2::run_compute_and_drain()
{
    this->process_compute();
    std::memset(this->w_buffer, 0, this->ce_width * (this->ce_pipe + 1) * this->bandwidth);
    this->compute_pending = false;
...
void LightRedmuleV2::handle_response(int slot_id, int64_t free_cycle)
{
...
vp::IoRespAck LightRedmuleV2::tcdm_response(vp::Block *__this, vp::IoReq *req)
{
    LightRedmuleV2 *_this = (LightRedmuleV2 *)__this;
    int slot_id = _this->req_slot_index[req];
    uint32_t instr = _this->req_slots[slot_id].instr;

    _this->handle_response(slot_id, _this->clock.get_cycles());

    _this->trace.msg(vp::Trace::LEVEL_TRACE, "[LightRedmuleV2][Resp] slot=%d instr=%d (w_out=%d, compute_pending=%d)\n",
        slot_id, instr, _this->w_outstanding_count, (int)_this->compute_pending);
    return vp::IO_RESP_ACCEPTED;
}

// io_v2 retry: the slave (interleaver) can accept again. Re-send the held,
// previously-DENIED request SYNCHRONOUSLY (the zero-buffer TCDM contract),
// then lift the issue block so the FSM resumes.
void LightRedmuleV2::tcdm_retry(vp::Block *__this, vp::IoRetryChannel)
{
    LightRedmuleV2 *_this = (LightRedmuleV2 *)__this;
    if (!_this->request_denied || _this->denied_slot_id < 0) return;

    int slot_id = _this->denied_slot_id;
    ReqSlot &slot = _this->req_slots[slot_id];
    slot.req->set_addr(slot.orig_addr);   // restore issue-time addr for the re-send
    slot.req->prepare();
    ...
    if (err == vp::IO_REQ_DONE)
    {
        ...
        _this->denied_slot_id = -1;
        int64_t free_cycle = _this->clock.get_cycles() + (int64_t)slot.req->get_full_latency();
    ...
    // IO_REQ_DENIED: still contended; keep the slot held for the next retry().
    _this->trace.msg(vp::Trace::LEVEL_TRACE, "[LightRedmuleV2][Retry] re-sent slot %d, status %d\n", slot_id, (int)err);
}
...
bool LightRedmuleV2::issue_request(uint32_t addr, uint32_t instr, bool is_write, uint32_t size)
{
    ...
    // prepare() resets per-send fields (latency/status). io_v2 correlates the
    // response back to this slot via req_slot_index (no per-req args).
    slot.orig_addr = addr;
    slot.req->prepare();
    ...
    if (err == vp::IO_REQ_DONE)
    {
        // Sync: data is back, but the port was busy for `latency` cycles.
    ...
    {
        // io_v2 DENIED = rejected. Hold this slot and re-send it synchronously
        // in tcdm_retry(); block further issues until then.
        this->request_denied = true;
        this->denied_slot_id = slot_id;
    }
    else
    {
        this->trace.fatal("[LightRedmuleV2] Unexpected IoReq status %d\n", (int)err);
        return false;
    }
...
void LightRedmuleV2::fsm_handler(vp::Block *__this, vp::ClockEvent *event)
{
    LightRedmuleV2 *_this = (LightRedmuleV2 *)__this;
    ...
    switch (_this->reg_fsm_state.get()) {
        ...
        case PRELOAD: {
            ...
            if (can_issue)
            {
                ...
                {
                    _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2][Preload] Send TCDM req #%d [addr=0x%08x]\n",
        ...
        case ROUTINE: {
            ...
            if (can_issue)
            {
                ...
                if (_this->issue_request(temp_addr, _this->iter_instruction, is_w, sz))
                {
                    _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2][ROUTINE-ijk: %0d-%0d-%0d] Send TCDM req #%d [addr=0x%08x]\n",
            ...
            if (iter_done)
            {
                ...
                {
                    _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2][ROUTINE-ijk: %0d-%0d-%0d] TCDM Access Time(%d) > Model Time(%d)\n", _this->iter_i, _this->iter_j, _this->iter_k, _this->fsm_timestamp, modeled_runtime);
                    latency = 1;
                } else {
                    _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2][ROUTINE-ijk: %0d-%0d-%0d] Model Time(%d) > TCDM Access Time(%d)\n",  _this->iter_i, _this->iter_j, _this->iter_k, modeled_runtime, _this->fsm_timestamp);
        ...
        case STORING: {
            ...
            if (can_issue)
            {
                ...
                if (_this->issue_request(temp_addr, _this->iter_instruction, is_w, sz))
                {
                    _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2][Storing] Send TCDM req #%d [addr=0x%08x]\n",
                        _this->fsm_counter, temp_addr);
                    _this->fsm_counter += 1;
                }
            }
            ...
            if (iter_done)
            {
                ...
                _this->num_matmul      += 1;
                _this->trace.msg("[LightRedmuleV2] Finished : %0d ns ---> %0d ns | period = %0d ns (%0d cyc) | uti = %0.3f | runtime = %0d ns | GEMM id = %0d | FMT = %0d | M-N-K = %0d-%0d-%0d | X-W-Y = 0x%5x-0x%5x-0x%5x\n",
        ...
        case FINISHED: {
            ...
            if (_this->redmule_query == NULL)
            {
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2][FINISHED] GRANT core and Wait for query!\n");
            } else {
        ...
        case ACKNOWLEDGE: {
            // Unreachable in this io_v2 variant: the CSR req_meth always answers
            // synchronously (IO_REQ_DONE), so redmule_query is never latched.
            _this->input_itf_v2.resp(_this->redmule_query);
            ...
            // };
            // _this->offload_grant_itf.sync(&offload_grant);
            _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmuleV2][ACKNOWLEDGE] GRANT core and Done!\n");
            _this->event_enqueue(_this->fsm_event, 1);
            break;
        }
        default:
            _this->trace.fatal("[LightRedmuleV2] INVALID RedMule Status: %d\n", _this->reg_fsm_state.get());
    }
}
...
void LightRedmuleV2::matmul_fp16(fp16 * z, fp16 * y, fp16 * x, fp16 * w, uint16_t m_size, uint16_t n_size, uint16_t k_size){
    for (int i = 0; i < m_size; ++i)
    {
        for (int j = 0; j < k_size; ++j)
        {
