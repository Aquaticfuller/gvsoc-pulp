...
private:
    ...
    std::map<vp::IoReq *, std::pair<L1NocFlit *, int>> tcdm_outstanding;
...
vp::IoReqStatus L1_NocItf::handle_noc_req(L1NocFlit *flit, int port)
{
    ...
    {
        this->tcdm_outstanding[req] = {flit, port};
    }
...
vp::IoRespAck L1_NocItf::tcdm_response(vp::Block *__this, vp::IoReq *req, int port)
{
    ...
    L1NocFlit *flit = entry->second.first;
    int request_port = entry->second.second;
