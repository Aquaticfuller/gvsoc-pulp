...
from pulp.teranoc_spatz.l1_interconnect.teranoc_l1_shim import TeranocL1Shim
from pulp.teranoc_spatz.l1_interconnect.teranoc_l1_xbar import TeranocL1Xbar
from pulp.teranoc_spatz.l1_interconnect.teranoc_l1_tcdm_bank_interco import TeranocL1TcdmBankInterco
from pulp.teranoc_spatz.l1_interconnect.teranoc_l1_burst_expander import TeranocL1BurstExpander
...
class L1_subsystem(gvsoc.systree.Component):
    ...
    def __init__(self, parent: gvsoc.systree.Component, name: str,
            ...
            burst_enable: bool = False, burst_local_issue: int = 1,
            burst_remote_issue: int = 3, burst_interleave: bool = True):
        ...
        # Burst expanders (tcdm_burst_expander): local ports get the narrow
        # (issue_width=1) variant, boundary ports the wide one. Each lane of a
        # boundary expander occupies its own bank-interco narrow input.
        boundary_lanes = burst_remote_issue if burst_enable else 1
        nb_narrow_inputs = nb_local_ports + nb_boundary_ports * boundary_lanes
        ...
        bank_interco = TeranocL1TcdmBankInterco(self, 'bank_interco',
            nb_narrow_inputs=nb_narrow_inputs, nb_banks=nb_banks_per_tile,
        ...
        remote_interco = TeranocL1Xbar(self, 'remote_interco', bandwidth=bandwidth,
            ...
            # Bursts cross as one header flit; responses stream back per word.
            header_flit=burst_enable,
            resp_beat_width=bandwidth if burst_enable else 0)
        ...
        for i in range(nb_local_ports):
            ...
            if burst_enable:
                # RTL i_local_burst_expander (IssueWidth=1) on the local path.
                local_exp = TeranocL1BurstExpander(self, f'local_burst_exp{i}',
                    issue_width=burst_local_issue, interleave=burst_interleave)
                shim.o_TCDM_LOCAL(local_exp.i_INPUT())
                assert burst_local_issue == 1
                local_exp.o_OUTPUT(0, bank_interco.i_NARROW(i))
            else:
        ...
        for kind, base, count in (('intra_group', 0, nb_intra_group_ports),
            ...
            for i in range(count):
                boundary_idx = base + i
                if burst_enable:
                    # RTL i_remote_burst_expander (IssueWidth = nr response
                    # ports): one bank-interco narrow input per issue lane.
                    exp = TeranocL1BurstExpander(self, f'{kind}_burst_exp{i}',
                        issue_width=burst_remote_issue, interleave=burst_interleave)
                    self.itf_bind(f'{kind}_in_{i}', exp.i_INPUT(),
                        signature=IoV2SingleReq(), composite_bind=True)
                    for lane in range(burst_remote_issue):
                        exp.o_OUTPUT(lane, bank_interco.i_NARROW(
                            nb_local_ports + boundary_idx * boundary_lanes + lane))
                else:
                    self.itf_bind(f'{kind}_in_{i}', bank_interco.i_NARROW(
                        nb_local_ports + boundary_idx),
                        signature=IoV2SingleReq(), composite_bind=True)
                remote_interco.o_OUTPUT(boundary_idx, gvsoc.systree.SlaveItf(
