      ...
      router_input_itf(LOCAL_PORT, &TeranocL1NocNetworkInterface::router_req)
{
    ...
    this->max_burst_size = this->get_js_config()->get_int("max_burst_size");
    if (this->max_burst_size <= 0)
    {
        this->max_burst_size = this->width;
    }
...
vp::IoReqStatus TeranocL1NocNetworkInterface::input_req(vp::Block *__this, vp::IoReq *req)
{
    ...
    // A burst request is one header flit (the RTL burst_len rides in the
    // header; there is no read payload), so up to max_burst_size bytes are a
    // single physical object on the mesh.
    vp_assert(flit->get_size() <= _this->width || flit->get_size() <= (uint64_t)_this->max_burst_size,
        &_this->trace,
        "TeraNoC L1 flit size 0x%lx exceeds lane width 0x%lx (max burst 0x%x)\n",
        flit->get_size(), _this->width, _this->max_burst_size);
