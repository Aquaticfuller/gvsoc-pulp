    ...
    while(burst_size > 0)
    {
        ...
        *router_req->arg_get(FlooNoc::REQ_WIDE) = (void *)wide;
        ...
        if (is_req)
        {
            ...
            {
                // Be careful to not have any request which is crossing 2 entries. For a
                // periodic entry, burst_base may be many periods past entry->base — use the
                // offset within the matched period (not raw burst_base) or this underflows.
                uint64_t rel = (entry->period > 0)
                    ? (burst_base - entry->base) % entry->period
                    : (burst_base - entry->base);
                uint64_t max_size = entry->size - rel;
